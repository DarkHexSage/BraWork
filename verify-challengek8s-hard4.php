...
function verifyConfigMapScenario() {
    ...
    $configMapOutput = shell_exec("sudo /snap/bin/microk8s.kubectl get configmap app-config -n configspace -o=jsonpath='{.metadata.name}' 2>&1");

    // Check if Pod 'app-pod' exists in namespace 'configspace'
    $podOutput = shell_exec("sudo /snap/bin/microk8s.kubectl get pod app-pod -n configspace -o=jsonpath='{.metadata.name}' 2>&1");
